...
// Web service tipo rest la tabla usuarios de la bd
...
switch($op){
    ...
    break;
    case 'PUT':
        //Codigo para actualizacion UPDATE
        ...
        && isset($_GET['tipo']) && isset($_GET['nombre'])){
            ...
            $t=$_GET['tipo'];
            $n=$_GET['nombre'];
            $cmd = $con->prepare("UPDATE usuarios SET pass=:p,tipo=:t,numbre=:n WHERE user=:u");
            $cmd->bindValue(':u',$u);
            $cmd->bindValue(':p',$p);
            $cmd->bindValue(':t',$t);
            $cmd->bindValue(':n',$n);
    ...
    break;
    case 'DELETE':
        //Codigo para eliminacion Delete
        if (isset($_GET['user'])){
            $u=$_GET['user'];
