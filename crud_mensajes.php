...
if($cmd->rowCount()>0){
    ...
    case "R":
        $idtema=$_GET['idtema'];
        $cmd = $con->prepare("SELECT idmsg as id, idtema as tema,mensaje,user as usuario  FROM mensajes WHERE idtema=:id");
        $cmd->bindValue(':id',$idtema);
    ...
    case "D":
        $id=$_REQUEST["idmsg"];
        $cmd = $con->prepare("DELETE FROM mensaje WHERE idmsg=:id");
        $cmd->bindValue(':id',$id);
        $cmd->execute();
        if($cmd->rowCount()>0){
            echo json_encode(["resp"=>"si"]);
